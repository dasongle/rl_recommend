...
def format_feature_space():
    ...
    # 3. feature
    feature_space = [[] for _ in xrange(size_user)]
...
def initialize_environment(sys_arg):
    ...
    sim_user_reward = [{} for _ in xrange(num_test)]
    user_avg_reward = [[] for _ in xrange(num_test)]
    click_rate = [[] for _ in xrange(num_test)]
...
def compute_average_reward(sim_vali_user, sim_user_reward, current_best_reward):
    user_avg_reward = []
    clk_rate = []
    for j in xrange(len(sim_vali_user)):
...
def sample_new_states(sim_vali_user, states, transition_p, reward_u, sim_user_reward, feature_space,  best_action_id, _k):
    ...
    sim_vali_user = [sim_vali_user[j] for j in xrange(previous_size) if j not in remove_set]
...
def sample_new_states_v2(sim_vali_user, states, transition_p, reward_u, sim_user_reward, feature_space,  best_action_id, _k):
    ...
    previous_size = len(sim_vali_user)
    states_removed = [states[j] for j in xrange(previous_size) if j not in remove_set]
    sim_vali_user_removed = [sim_vali_user[j] for j in xrange(previous_size) if j not in remove_set]
...
def sample_new_states_for_train(training_user, states, transition_p, reward_u, feature_space,  best_action_id, _k):
    ...
    states_removed = [states[j] for j in xrange(previous_size) if j not in remove_set]
    training_user_removed = [training_user[j] for j in xrange(previous_size) if j not in remove_set]
