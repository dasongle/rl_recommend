...
def construct_p():
    ...
    click_history = [[] for _ in xrange(_weighted_dim)]
    position_weight = [[] for _ in xrange(_weighted_dim)]
...
def construct_Q_and_loss():
    ...
    action_k_id = [[] for _ in xrange(_k)]
    for ii in xrange(_k):
        action_k_id[ii] = tf.placeholder(dtype=tf.int64, shape=[None])
    # (3) action features
    action_k_feature_gather = [[] for _ in xrange(_k)]
    for ii in xrange(_k):
    ...
    concate_input_k = [[] for _ in xrange(_k)]
    action_feature_list = []
    q_value_k = [[] for _ in xrange(_k)]
    loss_k = [[] for _ in xrange(_k)]
    opt_k = [[] for _ in xrange(_k)]
    train_variable_k = [[] for _ in xrange(_k)]
    train_op_k = [[] for _ in xrange(_k)]
...
def construct_max_Q():
    ...
    max_action_k = [[] for _ in xrange(_k)]
    max_action_feature_k = [[] for _ in xrange(_k)]
...
def form_init_Q_feed_dict(user_set, states_id):
    ...
    states_feature = [[] for _ in xrange(len(user_set))]
    history_order = [[] for _ in xrange(len(user_set))]  # np.zeros([len(user_set)], dtype=np.int64)
    history_user = [[] for _ in xrange(len(user_set))]  # np.arange(len(user_set), dtype=np.int64)

    for uu in xrange(len(user_set)):
        user = user_set[uu]
...
def test_during_training(current_best_reward):
    ...
    states = [[] for _ in xrange(len(sim_vali_user))]
    sim_u_reward = {}

    for t in xrange(_time_horizon):
...
def repeated_test(_repeated_best_reward, n_test):
    sim_user_reward = [{} for _ in xrange(n_test)]
    user_avg_reward = [[] for _ in xrange(n_test)]
    click_rate = [[] for _ in xrange(n_test)]
    mean_user_avg_reward = np.zeros(n_test)
    mean_click_rate = np.zeros(n_test)

    for i_th in xrange(n_test):
        sim_vali_user = _users_to_test
        states = [[] for _ in xrange(len(sim_vali_user))]
...
def form_max_q_feed_dict(user_set, states_id):
    ...
    # action_indicate_u = [[] for _ in xrange(len(user_set))]
    ...
    candidate_action_mean = [[] for _ in xrange(len(user_set))]
    candidate_action_std = [[] for _ in xrange(len(user_set))]

    action_cnt = [0 for _ in xrange(len(user_set))]

    action_space_cnt = [0 for _ in xrange(len(user_set))]
    ...
    for uu in xrange(len(user_set)):
        ...
        action_user_indice += [uu for _ in xrange(action_cnt[uu])]
...
def form_loss_feed_dict(user_set, states_id, action_id):
    ...
    action_ids_k = [[] for _ in xrange(_k)]
...
states = [[] for _ in xrange(len(training_user))]
...
for t in xrange(_time_horizon):
    ...
    random_action = [[] for _ in xrange(len(training_user))]
    random_action_feature = []
    for u_i in xrange(len(training_user)):
    ...
    # 5. sample new states
    remove_set = []
    for j in xrange(len(training_user)):
    ...
    states = [states[j] for j in xrange(previous_size) if j not in remove_set]
    training_user = [training_user[j] for j in xrange(previous_size) if j not in remove_set]
...
for n in xrange(batch_iterations):
